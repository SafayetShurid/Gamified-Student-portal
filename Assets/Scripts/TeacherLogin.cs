...
public class TeacherLogin : MonoBehaviour
{
    ...
    public void Login()
    {
        ...
        if (CheckLoginCredentials(email, password))
        {
            ...
            DatabaseManager.instance.SetCurrentTeacher(email);
            SceneManager.LoadScene("TeacherPanel");
    ...
    public bool CheckLoginCredentials(string email, string password)
    {
        ...
        using (var connection = new SqliteConnection(dbName))
        {
            ...
            command.CommandText = $"SELECT COUNT(*) FROM Teachers WHERE Email = @Email AND Password = @Password";
